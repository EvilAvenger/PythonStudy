...
def extract_names(filename):
    ...
    splitRegExp = re.compile(
        r'(?:<(?:td|th).*\s*(?:<b>)?([^<]+)(?:</b>)?(?:<b>)?(?:[^<]+)(?:</b>)?\s*</(?:td|th)>\s*)')
    file = open(filename, 'r', encoding="UTF-8")
    splitted = []
    myDict = dict()

    for tr in re.findall(regExp, file.read()):
        match = re.findall(splitRegExp, tr)
        splitted.append(list(map(str.strip, match)))

    for lst in splitted[2:]:
        myDict[lst[1]] = lst[2:]

    years = list(map(lambda x: int(x[0:4]), splitted[1]))
    return [years, myDict]


def print_years(years):
    year = 0
    print("Please specify year. Possible options: ")
    ...
    while True:
        ...
        try:
            ...
            print("Only numbers in XXXX format are allowed.")
        else:
            if year in years:
                break
            else:
                print(
                    "Year not found. Please specify another date from the list above: ")
    return years.index(year)


def print_names(babynames):
    index = print_years(babynames[0])
    for key in sorted(babynames[1]):
        print(": ".join((key, babynames[1][key][index])))
    return
