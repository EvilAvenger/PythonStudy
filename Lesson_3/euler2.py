...
def diff():  # 2016.02.02_09:01:59 checked. prusanov

    pows = sum(map(lambda x: x**2, range(1, 101)))
    pow = sum(range(1, 101))**2
    difference = pow - pows

    pows = sum([i * i for i in range(1, 101)])
    difference2 = pow - pows
...
def max_five():
    ...
    n = number.strip()
    while i <= len(n) - 5:
        myList.append(list(n[i:5 + i]))
        i += 1
    nums = map(lambda myNumberArray: reduce(
        lambda p1, p2: int(p1) * int(p2), myNumberArray), myList)
    return max(nums)
...
def summm():
    number = 2 << 999
    sum = findSumOfNumbersFromInt(number)
    return sum
...
def factorial():
    ...
    sum = findSumOfNumbersFromInt(num)
    return sum


def findSumOfNumbersFromInt(number):
    sum = reduce(lambda x, y: int(x) + int(y), list(str(number)))
    return sum
